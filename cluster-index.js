...
const WorkerQuantity = process.env.WORKER_QUANTITY;

const cluster = require('cluster');

if (cluster.isMaster) {
    for (let i = 0; i < WorkerQuantity; i++) {
        const workerEnv = { workerId: i };
    ...
    insertConfig = async (_req, _res) => {
        ...
        await _ControllerAPI.GetInsertConfig(local_time, parseISO, sending, _req.body.data_source, _req.body.campaign_name, data_leads, is_scheduled, site_id)
            .then(async function (response) {
                console_log(JSON.stringify({ 'statusCode': 200, 'status': true, message: 'Config Added', 'data': [], 'workerID': parseInt(process.env.NODE_APP_INSTANCE) }));
                _res.status(200).json({ 'statusCode': 200, 'status': true, message: 'Config Added', 'data': [], 'workerID': parseInt(process.env.NODE_APP_INSTANCE) });
            });
    }
