...
exports.API_DisplayTriggers = async (_req, _res) => {
    ...
    try {
        ...
        let query = `SELECT config_id,CASE WHEN (start_at AT TIME ZONE 'UTC' AT TIME ZONE 'GMT-8' > current_timestamp AND is_scheduled = true AND status = 'sending') THEN 'scheduled' WHEN (start_at AT TIME ZONE 'UTC' AT TIME ZONE 'GMT-8' < current_timestamp AND is_scheduled = true AND status = 'sending') THEN 'sending' ELSE status END AS status,triggerstatus,created_at,created_by,data_source,campaign_name,is_scheduled,start_at FROM cmw_config`;
...
exports.API_ViewHistory = async (_req, _res) => {
    local_connection.query(`
        SELECT 
        CASE
        WHEN cc.callback_status != 'Pending' THEN cc.api_response
        ELSE ch.api_response
        END AS api_response,ch.from_sender,ch.email_subject,ch.message,
        CASE 
        WHEN cc.callback_status != 'Pending' THEN cc.callback_status
        ELSE ch.status
        END AS status,ch.template_id,ch.application_id FROM cmw_history ch
