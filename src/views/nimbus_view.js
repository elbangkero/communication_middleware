...
exports.API_DisplayTriggers = async (_req, _res) => {
    ...
    try {
        ...
        let query = `select config_id,CASE WHEN (start_at AT TIME ZONE 'UTC' AT TIME ZONE 'GMT-8' > current_timestamp AND is_scheduled = true AND status = 'sending') THEN 'scheduled' ELSE status END AS status,triggerstatus,created_at,data_source,campaign_name,is_scheduled,start_at from cmw_config`;
